        ...
        <Box borderRadius='15px' background='white' boxShadow='0px 3px 6px #0000000A'>
            ...
            <HStack py={3} px={6} spacing={6} align='stretch'>
                {attemptsData.length ? attemptsData.slice(0, 4).map((ad) => {
                    ...
                    return (
                        <VStack key={ad._id}
                            bg={validityExpired && 'gray.100'}
                            cursor={!validityExpired && 'pointer'}
                            p={3} w={'25%'} align='stretch' borderRadius='6px' overflow='hidden'
                            boxShadow='rgba(17, 17, 26, 0.05) 0px 1px 0px, rgba(17, 17, 26, 0.1) 0px 0px 8px'
                            onClick={() => !validityExpired && openPkg(pkg)}
                        >
                            <Box flexGrow={1}>
    ...
    useEffect(() => {
        if (student?.packages?.length) {
            let tests = _.chain(student?.packages)
                .flatMap(p => p.package?.tests.map(t => ({ ...t, package: p })))
                .compact()
                .value()
            ...
            <HStack py={2} px={6} spacing={6} align='stretch'>
                {testAttempts?.length && pkgTests?.length ?
                    testAttempts.map(atmpt => {
                        ...
                        const validityExpired = test.package?.validity?.date ? moment(test.package.validity.date).isBefore(moment()) : false
                        return (
                            <VStack justifyContent='space-between' align='stretch' p={3} w={'25%'} borderRadius='6px' overflow='hidden'
                                boxShadow='rgba(17, 17, 26, 0.05) 0px 1px 0px, rgba(17, 17, 26, 0.1) 0px 0px 8px'
                                cursor={!validityExpired && 'pointer'}
                                onClick={() => !validityExpired && openTest(test.package?.package._id)}
                                bg={validityExpired && 'gray.100'}
                            >
                                <Box flexGrow={1}>
